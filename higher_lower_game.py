...
def get_random():
    random_entity = data[random.randint(0, len(data) - 1)]
    return random_entity


def get_higher_lower(first, second):
    if first > second:
        return 'a'
    else: 
        return 'b'
...
def compare_data(first_data, second_data):
    ...
    higher = get_higher_lower(first_data['follower_count'], second_data['follower_count'])
...
def start_game():
    first_data = get_random()
    second_data = get_random()
    compare_data(first_data, second_data)
